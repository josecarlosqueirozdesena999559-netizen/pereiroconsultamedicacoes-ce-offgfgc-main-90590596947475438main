...
serve(async (req) => {
  ...
  try {
    ...
    if (!GOOGLE_AI_KEY) {
      throw new Error("GOOGLE_AI_KEY não está configurada");
    }
          ...
          contents: [
            {
              parts: [
                ...
                {
                  text: `${systemPrompt}\n\nAnalise este PDF de estoque e encontre TODOS os medicamentos que contenham "${queryNormalized}" no nome. Retorne os dados em formato JSON.`,
                },
              ],
            },
          ],
